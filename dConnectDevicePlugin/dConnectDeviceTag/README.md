...
このプラグインでは、1度に1枚の QR コードしか読み込めませんので、配列が１つの tags が返却されます。
...
イベントが開始された後は、このターミナルにイベントが送られてキアmす。
...
## NFC フィルターについて
Tag プラグインでは、AndroidManifest.xml に下記のように NFC のフィルタリングを行なっています。
...
ですが、全てのフィルタリングを行うと Android OS が持つ「NFCサービス」と競合してしまい、毎回起動 Activity の選択ダイアログが表示されてしまいます。
