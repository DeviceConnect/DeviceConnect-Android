...
public class MainActivity extends AppCompatActivity {
    ...
    public void onConfigurationChanged(@NonNull Configuration newConfig) {
        ...
        if (mCamera2 != null) {
            ...
            if (mSRTClient != null && mSRTClient.getVideoEncoder() != null) {
                mSRTClient.restartVideoEncoder();
                CameraSurfaceVideoEncoder encoder = (CameraSurfaceVideoEncoder) mSRTClient.getVideoEncoder();
    ...
    private void startStreaming() {
        ...
        if (mSRTClient != null) {
            if (DEBUG) {
                Log.w(TAG, "SRTClient is already started.");
            }
            return;
        }
        ...
        String uri = mSettings.getBroadcastURI();

        if (uri == null) {
            uri = "srt://192.168.11.6:12345";
        }

        mSRTClient = new SRTClient(uri);
        ...
        mSRTClient.setOnEventListener(new SRTClient.OnEventListener() {
            ...
            public void onStarted() {
                Log.i(TAG, "SRTClient::onStarted");
            }

            @Override
            public void onStopped() {
                Log.i(TAG, "SRTClient::onStopped");
            }

            @Override
            public void onError(MediaEncoderException e) {
                Log.e(TAG, "SRTClient::onError", e);
            }

            @Override
            public void onConnected() {
                Log.i(TAG, "SRTClient::onConnected");
            }

            @Override
            public void onDisconnected() {
                Log.i(TAG, "SRTClient::onDisconnected");
            }

            @Override
            public void onNewBitrate(long bitrate) {
                Log.i(TAG, "SRTClient::onNewBitrate: " + bitrate);
            }
        });
        mSRTClient.start();
    }
