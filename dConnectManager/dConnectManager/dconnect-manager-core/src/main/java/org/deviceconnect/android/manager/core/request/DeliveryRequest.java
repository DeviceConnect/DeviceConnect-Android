...
public class DeliveryRequest extends LocalOAuthRequest {
    ...
    protected void onResponseReceived(final Intent request, final Intent response) {
        ...
        if (mRetryCount < MAX_RETRY_COUNT && errorCode == DConnectMessage.ErrorCode.NOT_FOUND_CLIENT_ID.getCode()) {
            ...
            executeRequest();
        } else if (mRetryCount < MAX_RETRY_COUNT && errorCode == DConnectMessage.ErrorCode.EXPIRED_ACCESS_TOKEN.getCode()) {
            // アクセストークンの有効期限切れ
