		...
		if (frameMode) {
			...
			for ( ; LIKELY(isRunning()) ; ) {
				...
				if (LIKELY(frame_mjpeg)) {
					...
					rewrite_jpeg_metadata(frame_mjpeg, frame_mjpeg->width, frame_mjpeg->height);

					// MODIFIED
					do_preview_pass_through(env, frame_mjpeg);
...
void UVCPreview::rewrite_jpeg_metadata(uvc_frame_t * frame, size_t width, size_t height) {
	ENTER();
	int i;
	size_t dc_huff_tbl_0_size = 33;
	size_t ac_huff_tbl_0_size = 183;
	size_t dc_huff_tbl_1_size = 33;
	size_t ac_huff_tbl_1_size = 183;
	size_t huff_tbl_size = dc_huff_tbl_0_size
						   + ac_huff_tbl_0_size
						   + dc_huff_tbl_1_size
						   + ac_huff_tbl_1_size;
	size_t data_bytes = frame->data_bytes + huff_tbl_size;

	static const UINT8 huff_tbl[432] = {
			// dc_huff_tbl_0
	...
	data[0] = ((unsigned char *) frame->data)[0];
	data[1] = ((unsigned char *) frame->data)[1];
	for (i = 0; i < huff_tbl_size; i++) {
		data[i + 2] = huff_tbl[i];
	}
	for (i = 2; i < frame->data_bytes; i++) {
		data[i + huff_tbl_size] = ((unsigned char *) frame->data)[i];
	}
	free(frame->data);
