...
import org.deviceconnect.android.rtspserver.BuildConfig;
...
public class ScreenCastVideoStream extends VideoStream {
    ...
    public ScreenCastVideoStream(Context context) {
        mContext = context;
        mVideoEncoder = new ScreenCastVideoEncoder(mContext);
    }
    ...
    public RtpPacketize createRtpPacketize() {
        ...
        packetize.setPayloadType(96);
        packetize.setClockFrequency(90000);
        return packetize;
    }
    ...
    public void configure() {
        if (mPPSString == null || mSPSString == null) {
            ...
            VideoQuality quality = mVideoEncoder.getVideoQuality();

            int w = quality.getVideoWidth();
            int h = quality.getVideoHeight();

            VideoEncoder encoder = new TempVideoEncoder();
            encoder.getVideoQuality().set(mVideoEncoder.getVideoQuality());
