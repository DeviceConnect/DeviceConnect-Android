...
public class SRTSocketThread {
     ...
     */
    public SRTSocketThread(String address, int port) {
        mAddress = address;
        mPort = port;
    }
     ...
     */
    public void addSocketOptions(Map<Integer, Object> socketOptions) {
        mCustomSocketOptions.putAll(socketOptions);
    }
     ...
     * </p>
     *
     * @param key ソケットに反映するオプション
     * @param value ソケットに反映するオプションの値
     */
    public void addSocketOption(Integer key, Object value) {
        mCustomSocketOptions.put(key, value);
    }
     ...
     * データを送信します
     * @param data 送信するデータ
     * @param offset 送信するデータのオフセット
     * @param length 送信するデータの長さ
     * @throws SRTSocketException 送信に失敗した場合に発生
     */
    public void sendData(byte[] data, int offset, int length) throws SRTSocketException {
        if (isConnected()) {
            mSRTSessionThread.send(data, offset, length);
        } else {
    ...
    public synchronized boolean isConnected() {
        return mSRTSessionThread != null && mSRTSessionThread.isConnected();
    }
    ...
    private class SRTSessionThread extends Thread {
        ...
        /**
         * SRTSocket の接続状態を確認します.
         *
         * @return 接続されている場合はtrue、それ以外はfalse
         */
        boolean isConnected() {
            return mSRTSocket != null && mSRTSocket.isConnected();
        }

        /**
         * SRTSocket にデータを送信します.
         *
         * @param data 送信するデータ
         * @param offset 送信するデータのオフセット
         * @param length 送信するデータの長さ
         * @throws SRTSocketException 送信に失敗した場合に発生
         */
        void send(byte[] data, int offset, int length) throws SRTSocketException {
            mSRTSocket.send(data, offset, length);
        }
    ...
    private void postOnErrorConnecting(Exception e) {
        if (mOnEventListener != null) {
            mOnEventListener.onnErrorConnecting(e);
        }
    }
    ...
    public interface OnEventListener {
         ...
         * @param e 接続失敗原因の例外
         */
        void onnErrorConnecting(Exception e);
