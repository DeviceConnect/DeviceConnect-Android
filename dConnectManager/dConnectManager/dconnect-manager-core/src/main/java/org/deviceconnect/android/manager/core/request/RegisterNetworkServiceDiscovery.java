...
import org.deviceconnect.android.manager.core.event.EventProtocol;
...
public class RegisterNetworkServiceDiscovery extends DConnectRequest {
    ...
    public void run() {
        ...
        // リクエストを作成
        Intent request = EventProtocol.createRegistrationRequestForServiceChange(mContext, mDevicePlugin, mInterface.getDConnectBroadcastReceiverClass());
    ...
    private String getErrorMessage(final Intent response) {
        return response.getStringExtra(DConnectMessage.EXTRA_ERROR_MESSAGE);
    }
