...
//        setContentView(R.layout.activity_transparent);
        ...
        if (callingIntent == null || callingIntent.getExtras() == null ||
                !callingIntent.hasExtra(EXTRA_INTENT) ||
        ...
        mIntent = extras.getParcelable(EXTRA_INTENT);
        if (mIntent == null) {
            finish();
            return;
        }
        mCallback = extras.getParcelable(EXTRA_CALLBACK);
        if (mCallback == null) {
            finish();
            return;
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_CODE) {
            ...
            mCallback.send(resultCode, dataBundle);
            finish();
        }
    }

    public static void startActivityForResult(@NonNull final Context context,
        ...
        // NOTE: FLAG_ACTIVITY_SINGLE_TASK causes Activity#onActivityResult() being called
        // prematurely. FLAG_ACTIVITY_SINGLE_TOP, on the other hand, does not cause that.
